function out = sk_multilevel_pls_cv_adapted(X,Y,subj,ncomp,scaling, cvset)
    ...
    if scaling == true
        Xbstd_train = repmat(std(Xb_train),n_train,1);
        Ycstd_train = repmat(std(Yc_train),n_train,1);
        Xbsc_train = Xb_train./Xbstd_train;
    ...
    out.Xbstd_train = Xbstd_train;
    out.Xbsc_train = Xbsc_train;
    
    out.Ycstd_train = Ycstd_train;
    out.Ycsc_train = Ycsc_train;
    
    [Px_train, Py_train, Tx_train, Ty_train, beta_train] = plsregress(Xbsc_train, Ycsc_train, ncomp);
    ...
    out.pred_train = ([ones(n_train,1), out.Xbsc_train]*out.beta_train).*out.Ycstd_train + out.Ym_train;    
    out.error_train = 100*sum(Y_train ~= ((out.pred_train > 0) - (out.pred_train < 0)))/n_train;
    ...
    Xbsc_test = Xb_test./Xbstd_train(1:n_test,:);
    
    out.Xc_test = Xc_test;
    out.Xb_test = Xb_test;
    out.subj_test = subj_test;
    out.pred_test = ([ones(n_test,1), Xbsc_test]*out.beta_train).*out.Ycstd_train(1:n_test,1) + out.Ym_train(1:n_test,1);
    out.error_cv = sum(Y_test ~= ((out.pred_test > 0) - (out.pred_test < 0)));
    out.sq_error_cv = sum((Y_test - out.pred_test).^2);
    out.sq_error_mod_cv = sum((Y_test - (2./(1+exp(-2*out.pred_test))-1)).^2)/n_train;
